...
class Mail(Controller):
    ...
    @route_menu(list_name=u'backend', group=u'互動項目', need_hr=True, text=u'郵件寄送樣版', sort=701)
    def admin_list(self):
    ...
    def admin_reset_mail_list(self):
        m = self.meta.Model
        m.replace_context('notice_create_user_by_email',
u'帳號註冊完成通知信',
...
''')
        ...
        m.replace_context('send_token_to_reset_password',
u'忘記密碼通知信',
...
''')
        ...
        m.replace_context('order_create_send_to_admin',
u'訂購通知信',
...
''')
        ...
        m.replace_context('order_create_send_to_user',
u'訂購通知信',
u'{{ site_name }} 訂購通知信', u'''
